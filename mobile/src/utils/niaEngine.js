// FILE: mobile/src/utils/niaEngine.js
// Purpose: Nia AI Nutritionist response engine
// Reused from: web's lib/niaEngine.js — pure JS logic, no web APIs needed
...
const RESPONSES = {
  ...
  meal_plan: () => `Here's your personalized 7-day meal plan based on your profile.`,
...
export const getNiaResponse = (input, profileData, contextData = {}) => {
  ...
  const allergies =
    Array.isArray(profileData?.allergies) && profileData.allergies.length ? profileData.allergies : [];
  ...
  if (
    allergies.length > 0 &&
    ['meal_plan', 'protein', 'breakfast', 'snacks', 'weight_loss', 'muscle_gain'].includes(intent)
  ) {
  ...
  return {
    ...
    text: isMealPlan
      ...
      : isCourses
      ? 'Here are some great nutrition courses and resources:'
      : responseText,
